...
| Проблема | Решение |
|---|---|
| Дорогой старт | **Манифест-конституция** — 25 строк на роль, читается **один раз за сессию** |
| Повторное чтение | После задачи агент уходит в **пассивный режим** и ждёт следующего сообщения, не перечитывая себя |
...
Правится только `manifests/`. Загрузчики в `agents/` трогать не нужно: они лишь указывают,
что прочитать. После правки манифеста поднимите `version` в `plugins/ceo-team/.claude-plugin/plugin.json`,
иначе установленные копии не увидят обновление.
...
plugins/ceo-team/
├── agents/                         10 тонких загрузчиков (по 7 строк)
├── manifests/                      конституции: _core.md + 10 ролей
├── commands/start.md               опрос
├── hooks/hooks.json                запуск при старте сессии
├── scripts/session-start.sh        развёртывание папок и .gitignore
└── template/                       заготовки списков и RECOVERY
```

Правится всё в `manifests/`. Загрузчики трогать не нужно — они только указывают агенту, что прочитать.
