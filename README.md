...
Перезапустите сессию — плагин сам создаст рабочие папки и допишет `.gitignore`.
...
Короткий опрос: многозадачность, состав команды, модели. Ответы ложатся в
`claude-agents/SESSION.md`, и `ceo-agent` работает по ним. Команда доступна всегда —
ей же можно перенастроить сессию на ходу.

Дальше — обычная работа, всё через руководителя:
